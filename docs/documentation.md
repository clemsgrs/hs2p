...
- Preview saving now uses explicit `tiling.preview.save_mask_preview` and `tiling.preview.save_tiling_preview` booleans instead of the old top-level `save_previews` flag.
- The preview toggles independently control mask overlays and tiling previews.
