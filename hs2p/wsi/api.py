...
def preprocess_slide(*args, **kwargs):
    from hs2p.preprocessing import preprocess_slide as _preprocess_slide

    return _preprocess_slide(*args, **kwargs)


def _is_default_tissue_sampling_spec(
    sampling_spec: ResolvedSamplingSpec | None,
) -> bool:
    if sampling_spec is None:
        return True
    return (
        dict(sampling_spec.pixel_mapping) == DEFAULT_TISSUE_PIXEL_MAPPING
        and tuple(sampling_spec.active_annotations) == ("tissue",)
        and sampling_spec.tissue_percentage.get("background") is None
        and sampling_spec.tissue_percentage.get("tissue") is not None
    )


def _coordinate_result_from_shared_preprocessing(
    shared_result,
) -> CoordinateExtractionResult:
    coordinates = np.asarray(shared_result.coordinates, dtype=np.int64)
    ...
    resize_factor = float(shared_result.effective_tile_size_px) / float(
        shared_result.requested_tile_size_px
    )
    read_step_px = max(
        1,
        int(
            round(
                int(shared_result.effective_tile_size_px)
                * (1.0 - float(shared_result.overlap or 0.0)),
                0,
            )
        ),
    )
    return CoordinateExtractionResult(
        contour_indices=[0] * len(coordinates),
        tissue_percentages=np.asarray(
            shared_result.tissue_fractions,
            dtype=np.float32,
        ).tolist(),
        x=x,
        y=y,
        read_level=int(shared_result.read_level),
        read_spacing_um=float(shared_result.effective_spacing_um),
        read_tile_size_px=int(shared_result.effective_tile_size_px),
        read_step_px=read_step_px,
        resize_factor=resize_factor,
        tile_size_lv0=int(shared_result.tile_size_lv0),
        step_px_lv0=int(shared_result.step_px_lv0 or shared_result.tile_size_lv0),
    )


def _extract_coordinate_result_from_preprocessing(
    *,
    request: UnifiedCoordinateRequest,
) -> tuple[CoordinateExtractionResult, np.ndarray | None]:
    shared_result = preprocess_slide(
        image_path=request.wsi_path,
        tissue_mask_path=request.mask_path,
        tissue_mask_tissue_value=int(
            getattr(request.tiling_params, "tissue_mask_tissue_value", 1)
        ),
        backend=request.backend,
        spacing_override=request.spacing_at_level_0,
        requested_tile_size_px=request.tiling_params.target_tile_size_px,
        requested_spacing_um=request.tiling_params.target_spacing_um,
        use_hsv=request.segment_params.use_hsv,
        use_otsu=request.segment_params.use_otsu,
        sthresh=request.segment_params.sthresh,
        sthresh_up=request.segment_params.sthresh_up,
        mthresh=request.segment_params.mthresh,
        close=request.segment_params.close,
        min_tissue_fraction=request.tiling_params.tissue_threshold,
        overlap=request.tiling_params.overlap,
        use_padding=request.tiling_params.use_padding,
        seg_downsample=request.segment_params.downsample,
        tolerance=request.tiling_params.tolerance,
        ref_tile_size_px=request.filter_params.ref_tile_size,
        a_t=request.filter_params.a_t,
        a_h=request.filter_params.a_h,
        max_n_holes=request.filter_params.max_n_holes,
        filter_white=request.filter_params.filter_white,
        filter_black=request.filter_params.filter_black,
        white_threshold=request.filter_params.white_threshold,
        black_threshold=request.filter_params.black_threshold,
        fraction_threshold=request.filter_params.fraction_threshold,
        num_workers=request.num_workers,
        selection_strategy=request.selection_strategy,
        output_mode=request.output_mode,
    )
    return (
        _coordinate_result_from_shared_preprocessing(shared_result),
        shared_result.tissue_mask,
    )
...
def execute_coordinate_request(
    ...
    if (
        request.selection_strategy != CoordinateSelectionStrategy.INDEPENDENT_SAMPLING
        and _is_default_tissue_sampling_spec(request.sampling_spec)
    ):
        merged_result, preview_mask = _extract_coordinate_result_from_preprocessing(
            request=request,
        )
        if request.mask_preview_path is not None and preview_mask is not None:
            preview_mask_arr = _normalize_tissue_mask(preview_mask)
            if (
                request.preview_pixel_mapping is not None
                and request.preview_color_mapping is not None
            ):
                preview_mask_arr = np.where(preview_mask_arr > 0, 1, 0).astype(
                    np.uint8,
                    copy=False,
                )
            _save_overlay_preview(
                ...
                palette=request.preview_palette,
                pixel_mapping=request.preview_pixel_mapping,
                color_mapping=request.preview_color_mapping,
        ...
        if request.sampling_spec is None:
            raise ValueError("sampling_spec is required for sampling output")
        return UnifiedCoordinateResponse(
            per_annotation_results={"tissue": merged_result}
        )
