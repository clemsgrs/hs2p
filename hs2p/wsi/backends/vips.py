...
class VIPSReader:
    def __init__(self, path: str | Path, *, spacing_override: float | None = None):
        ...
        self._loader = (
            self._root.get("vips-loader") if "vips-loader" in self._root.get_fields() else None
        )
    ...
    def _open_level_image(self, level: int):
        if self._loader == "tiffload" and level is not None:
            return self._vips.Image.new_from_file(self._path, page=int(level))
        if level is not None and len(self._level_dimensions) > 1:
