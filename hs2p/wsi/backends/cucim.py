...
class CuCIMReader:
    def __init__(
        ...
        *,
        ...
        cucim_meta = self._metadata.get("cucim", {}) if isinstance(self._metadata, dict) else {}
        resolutions = cucim_meta.get("resolutions", {}) if isinstance(cucim_meta, dict) else {}
        ...
        if len(self._level_downsamples) != len(self._level_dimensions):
            self._level_downsamples = [
                (self._level_dimensions[0][0] / float(width), self._level_dimensions[0][1] / float(height))
    ...
    def read_region(
        ...
        *,
        ...
        if not pad_missing:
            return _as_rgb_uint8(
                self._read_region(location, size, level=int(level))
            )
        ...
        region = _as_rgb_uint8(
            self._read_region(read_location, read_size, level=int(level))
        )
