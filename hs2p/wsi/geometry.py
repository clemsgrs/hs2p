...
def select_level_for_downsample(
    ...
    return int(
        np.argmin(
            [abs(float(downsample[0]) - requested_downsample) for downsample in level_downsamples]
        )
    )
...
def select_level(
    *,
    ...
    level = int(
        np.argmin(
            [abs(effective_spacing - requested_spacing_um) for effective_spacing in level_spacings]
        )
    )
