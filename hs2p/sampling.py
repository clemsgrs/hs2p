...
from hs2p.wsi import sample_coordinates, save_coordinates, visualize_coordinates
...
def process_slide(
    *,
    ...
    try:
        for annotation in pixel_mapping.keys():
...
def main(args):
    ...
    pixel_mapping = {k: v for e in cfg.sampling.pixel_mapping for k, v in e.items()}
    if cfg.sampling.color_mapping is not None:
        color_mapping = {k: v for e in cfg.sampling.color_mapping for k, v in e.items()}
    else:
        color_mapping = None
    ...
    if not skip_sampling:
        ...
        print(f"No tiles after sampling step: {len(no_tiles)}")
        print("=+=" * 10)
