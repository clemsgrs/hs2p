...
# --- keyword-only construction (Finding 2) ----------------------------------------------


def test_positional_construction_raises_type_error():
    with pytest.raises(TypeError):
        TilingConfig(0.5, 224, 0.07, 0.0, {"tissue": 0.1}, "asap")
...
def test_keyword_construction_works():
    ...
    assert tiling.backend == "asap"


# --- requested_* validation (Finding 3) -------------------------------------------------


def test_explicit_bogus_requested_backend_fails():
    with pytest.raises(ValueError):
        _tiling(requested_backend="bogus")


def test_explicit_bogus_requested_mask_backend_fails():
    with pytest.raises(ValueError):
        _tiling(requested_mask_backend="bogus")


def test_explicit_valid_requested_backend_accepted():
    tiling = _tiling(requested_backend="asap")
    assert tiling.requested_backend == "asap"


def test_explicit_valid_requested_mask_backend_accepted():
    tiling = _tiling(requested_mask_backend="asap")
    assert tiling.requested_mask_backend == "asap"
