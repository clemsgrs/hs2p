...
def test_resolve_spacing_plan_resamples_directly_when_requested_spacing_exceeds_coarsest_native_level():
    wsi = _SpacingPlanWSI(
    ...
    level, read_spacing_um, requested_read_spacing_um, needs_resampling = _resolve_spacing_plan(
        wsi=wsi,
        requested_spacing_um=4.0,
        tolerance=0.05,
        spacing_at_level_0=None,
    )

    assert level == 3
    assert read_spacing_um == pytest.approx(1.92)
    assert requested_read_spacing_um == pytest.approx(4.0)
    assert needs_resampling is True


def test_resolve_spacing_plan_error_lists_requested_available_spacings_and_guidance():
    wsi = _SpacingPlanWSI(
        spacings=[0.24, 0.48, 0.96, 1.92],
        shapes=[(1024, 1024), (512, 512), (256, 256), (128, 128)],
    )
    ...
    with pytest.raises(ValueError) as exc_info:
        _resolve_spacing_plan(
            wsi=wsi,
            requested_spacing_um=0.20,
            tolerance=0.05,
            spacing_at_level_0=None,
        )

    message = str(exc_info.value).lower()
    assert "requested tissue-mask spacing (0.2" in message
